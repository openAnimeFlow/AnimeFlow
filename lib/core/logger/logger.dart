...
class LiggLogPrinter extends LogPrinter {
...
class LiggLogOutput extends LogOutput {
  ...
  static Future<String> _getLogFilePath() async {
    ...
    _logFilePath = p.join(logDir, "ligg_logs.log");
    return _logFilePath!;
  }
  ...
  void _writeToFile(OutputEvent event) {
    _logLock.synchronized(() async {
      try {
        ...
        stdout.writeln('Failed to write log to file: ${_singleLineLogText(e)}');
      }
    });
  }
...
class LiggLogger {
  ...
  static final LiggLogger _instance = LiggLogger._internal();
  factory LiggLogger() {
    return _instance;
  }
...
Future<File> getLogsPath() async {
  final dir = (await getApplicationSupportDirectory()).path;
  final logDir = p.join(dir, "logs");
  final filename = p.join(logDir, "ligg_logs.log");
  ...
  if (!await file.exists()) {
    await LiggLogOutput._logLock.synchronized(() async {
      if (!await file.exists()) {
        await file.create();
      }
    });
...
Future<bool> clearLogs() async {
  try {
    ...
    stdout.writeln('Error clearing file: ${_singleLineLogText(e)}');
    return false;
  }
}
